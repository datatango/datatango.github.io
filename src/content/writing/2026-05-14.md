---
title: "What I Learned About SSH by Fixing My Own Broken Setup"
date: "2026-04-29"
description: "Notes on setting up SSH"
---

I had two servers I ssh into all day. One of them (call it `dev-box`) had always been quiet and fast. Type `ssh dev-box`, you're in. The other (`wsl-host`, a WSL2 machine) greeted me like this every single time:
...
`UseKeychain yes` and `AddKeysToAgent yes` are the magic on macOS — first time you ssh, you enter your passphrase, it goes into Keychain, and you never see it again.
...
`ForwardAgent yes` tells your local ssh client to expose its agent socket to the remote session. Anything on the remote that needs to authenticate via SSH — a `git push`, a nested `ssh` to another host, an `scp` — uses *your local agent* through that socket. Your private key never leaves your laptop.

Caveat: if the remote host is compromised, an attacker with root could briefly use your forwarded agent. Fine for hosts you trust; not appropriate for shared or untrusted boxes.
